...
civSizes[civSizes.thorp      ] = { min_pop :      0, name : "村莊"       , id : "thorp"      };
civSizes.hamlet      =  civSizes.length; 
civSizes[civSizes.hamlet     ] = { min_pop :     20, name : "部落"      , id : "hamlet"     };
civSizes.village     =  civSizes.length; 
civSizes[civSizes.village    ] = { min_pop :     60, name : "鄉村"     , id : "village"    };
civSizes.smallTown   =  civSizes.length; 
civSizes[civSizes.smallTown  ] = { min_pop :    200, name : "小鎮"  , id : "smallTown"  };
civSizes.largeTown   =  civSizes.length; 
civSizes[civSizes.largeTown  ] = { min_pop :   2000, name : "城鎮"  , id : "largeTown"  };
civSizes.smallCity   =  civSizes.length; 
civSizes[civSizes.smallCity  ] = { min_pop :   5000, name : "小城"  , id : "smallCity"  };
civSizes.largeCity   =  civSizes.length; 
civSizes[civSizes.largeCity  ] = { min_pop :  10000, name : "大城"  , id : "largeCity"  };
civSizes.metropolis  =  civSizes.length; 
civSizes[civSizes.metropolis ] = { min_pop :  20000, name : "都市"  , id : "metropolis" };
civSizes.smallNation =  civSizes.length; 
civSizes[civSizes.smallNation] = { min_pop :  50000, name : "小國", id : "smallNation"};
civSizes.nation      =  civSizes.length; 
civSizes[civSizes.nation     ] = { min_pop : 100000, name : "國家"      , id : "nation"     };
civSizes.largeNation = civSizes.length; 
civSizes[civSizes.largeNation] = { min_pop : 200000, name : "大國", id : "largeNation"};
civSizes.empire      = civSizes.length; 
civSizes[civSizes.empire     ] = { min_pop : 500000, name : "帝國"      , id : "empire"     };
...
if (!read_cookie('civ') && !localStorage.getItem('civ')){
	var civName = prompt('請為你的文明取一個名字','');
	document.getElementById('civName').innerHTML = civName;
}
if (!read_cookie('civ') && !localStorage.getItem('civ')){
	var rulerName = prompt('你叫什麼名字?','');
...
function updateDeity(){
	if (upgrades.deity == 1){
		...
		if (deity.type) {
			document.getElementById('deity' + deity.seniority + 'Type').innerHTML = ", " + deity.type + "之神";
		} else {
...
function updateWonderList(){
	...
	if (wonder.total > 0){
		//update wonder list
		var wonderhtml = '<tr><td><strong>奇觀名</strong></td><td><strong>類別</strong></td></tr>';
...
function raiseDead(num){
	...
	if (population.corpses >= num && piety.total >= cost){
		...
		//Notify player
		if (num == 1) {
			gameLog("一具殭屍復活了, 聽你使喚.");
		} else {
			gameLog("一群殭屍復活了, 聽你使喚.");
		}
...
function plague(sickNum){
	...
	updatePopulation();
	console.log('瘟疫: ' + actualNum + '個工人染病.');
	gameLog(prettify(actualNum) + '個工人染病'); //notify player
...
function iconoclasmList(){
	...
	if (piety.total >= 1000){
		...
		append += '<br /><button onclick=\'iconoclasm("cancel")\'>取消</button>';
...
function spawnMob(mobtype){
	...
	if (mobtype == 'wolf'){
		...
		//Informs player
        if (clt > 0) { gameLog(prettify(clt) + '條野狼來襲'); }
		document.getElementById('wolfgroup').style.display = 'table-row';
	}
	if(mobtype == 'bandit'){
		num = Math.random();
        ...
        if (cls > 0){
			gameLog(prettify(clt) + '個土匪來襲, 一共有' + prettify(cls) + '個攻城塔');
			document.getElementById('banditgroup').style.display = 'table-row';
			document.getElementById('esiegegroup').style.display = 'table-row';
		} else if (clt > 0){
			gameLog(prettify(clt) + '個土匪來襲');
			document.getElementById('banditgroup').style.display = 'table-row';
		}
	}
	if (mobtype == 'barbarian'){
		...
		population.esiege += cls;
        if (cls > 0){
			gameLog(prettify(clt) + '個蠻族來襲, 一共有' + prettify(cls) + '個攻城塔');
			document.getElementById('barbariangroup').style.display = 'table-row';
			document.getElementById('esiegegroup').style.display = 'table-row';
		} else if (clt > 0){
			gameLog(prettify(clt) + '個蠻族來襲');
			document.getElementById('barbariangroup').style.display = 'table-row';
		}
	}
...
function smite(){
	...
	if (population.barbarians > 0){
		if (piety.total >= population.barbarians * 100){
			...
			gameLog('打到' + population.barbarians + '個蠻族');
	...
	if (population.bandits > 0){
		if (piety.total >= population.bandits * 100){
			...
			gameLog('打到' + population.bandits + '個土匪');
	...
	if (population.wolves > 0){
		if (piety.total >= population.wolves * 100){
			...
			gameLog('打到' + population.wolves + '條野狼');
...
// 	var prettyLast = (raiding.last == 'empire') ? 'an ' : 'a ';
	var prettyLast = (raiding.last == 'empire') ? '' : '';
	prettyLast += civSizes[civSizes[raiding.last]].name;

	var plunderMessage = "打敗" + prettyLast + ". 掠獲" + prettify(plunderFood) + "個食物, " + prettify(plunderWood) + "個木材, " + prettify(plunderStone) + "個石材, " + prettify(plunderSkins) + "個獸皮, " + prettify(plunderHerbs) + "個藥草, " + prettify(plunderOre) + "個礦石, " + prettify(plunderLeather) + "個皮革, 還有" + prettify(plunderMetal) + "個金屬. 總獲得" + prettify(landCaptured) + "個土地.";
	//add loot
	land += landCaptured;
	updateResourceTotals();
...
function renameWonder(){
	var n = prompt('請命名你的奇觀:',wonder.name);
	wonder.name = n;
	document.getElementById('wonderNameP').innerHTML = wonder.name;
	document.getElementById('wonderNameC').innerHTML = wonder.name;
}
...
function updateWonderLimited(){
	if (food.total < 1){
		document.getElementById('limited').innerHTML = "食物不足";
	} else if (wood.total < 1){
		document.getElementById('limited').innerHTML = "木材不足";
	} else if (stone.total < 1){
		document.getElementById('limited').innerHTML = "石材不足";
	} else if (skins.total < 1){
		document.getElementById('limited').innerHTML = "獸皮不足";
	} else if (herbs.total < 1){
		document.getElementById('limited').innerHTML = "藥草不足";
	} else if (ore.total < 1){
		document.getElementById('limited').innerHTML = "礦石不足";
	} else if (leather.total < 1){
		document.getElementById('limited').innerHTML = "皮革不足";
	} else if (piety.total < 1){
		document.getElementById('limited').innerHTML = "虔誠不足";
	} else if (metal.total < 1){
		document.getElementById('limited').innerHTML = "金屬不足";
	}
}
...
function trade(){
	...
	if (trader.material){
		...
		if (trader.material.total >= trader.requested){
			...
			gold.total += 1;
			updateResourceTotals();
			gameLog('交換了' + trader.requested + ' ' + trader.material.name);
...
function toggleAutosave(){
	...
	if (autosave == "on"){
		console.log("已切換到禁止自動保存");
		autosave = "off";
		document.getElementById("toggleAutosave").innerHTML = "啓用自動保存";
	} else {
		console.log("已切換到啓用自動保存");
		autosave = "on";
		document.getElementById("toggleAutosave").innerHTML = "禁止自動保存";
	}
}

function deleteSave(){
	//Deletes the current savegame by setting the game's cookies to expire in the past.
	var really = confirm('確定要刪除存檔嗎?'); //Check the player really wanted to do that.
	if (really){
...
function renameCiv(){
	...
	var n = prompt('請為你的文明取一個名字',civName);
	if (n !== null){
		civName = n;
		document.getElementById('civName').innerHTML = civName;
	}
...
function renameDeity(){
	...
	var n = prompt('人民拜哪一位神明?',deity.name);
	if (n !== null){
		deity.name = n;
		updateDeity();
	}
...
function reset(){
	...
	var really = confirm('確定要重置嗎? 重置後可以保留已有的神明和奇觀 (還有神貓)'); //Check player really wanted to do that.
	if (really){
		if (upgrades.deity == 1){
			if (oldDeities){
...
window.setInterval(function(){
	...
	if (food.total < 0) { //and will starve if they don't have enough
		if (upgrades.waste && population.corpses >= (food.total * -1)){ //population eats corpses instead
			...
			if (starve == 1) { gameLog('一個工人餓死了'); }
			if (starve > 1) { gameLog(prettify(starve) + '個工人餓死了'); }
			for (i=0; i<starve; i++){
				jobCull();
			}
			updateJobs();
	...
	if (population.wolves > 0){
		if (population.soldiers > 0 || population.cavalry > 0){ //FIGHT!
			...
			if (population.healthy > 0){
				...
				console.log('野狼把' + target + '吃掉');
                gameLog('野狼把' + target + '吃掉');
	...
	if (population.bandits > 0){
		if (population.soldiers > 0 || population.cavalry > 0){//FIGHT!
			...
			if (num < 1/8){
				if (food.total > 0) { gameLog('土匪搶劫了食物'); }
				...
				if (wood.total > 0) { gameLog('土匪搶劫了木材'); }
				...
				if (stone.total > 0) { gameLog('土匪搶劫了石材'); }
				...
				if (skins.total > 0) { gameLog('土匪搶劫了獸皮'); }
				...
				if (herbs.total > 0) { gameLog('土匪搶劫了藥草'); }
				...
				if (ore.total > 0) { gameLog('土匪搶劫了礦石'); }
				if (ore.total >= stolen){
					ore.total -= stolen;
				} else {
					ore.total = 0;
				...
				if (leather.total > 0) { gameLog('土匪搶劫了皮革'); }
				...
				if (metal.total > 0) { gameLog('土匪搶劫了金屬'); }
		...
		if (population.soldiers > 0 || population.cavalry > 0){//FIGHT!
			...
			if (havoc < 0.3){
				...
				if (population.healthy > 0){
					...
					console.log('蠻族殺害一個' + target);
					gameLog('蠻族殺害一個' + target);
					if (target == "unemployed"){
						population.current -= 1;
						population.unemployed -= 1;
					} else if (target == "farmer"){
				...
				stolen = Math.floor((Math.random() * 1000)); //Steal up to 1000.
				if (num < 1/8){
					if (food.total > 0) { gameLog('蠻族搶劫了食物'); }
					if (food.total >= stolen){
						food.total -= stolen;
					} else {
						food.total = 0;
					...
					if (wood.total > 0) { gameLog('蠻族搶劫了木材'); }
					if (wood.total >= stolen){
						wood.total -= stolen;
					} else {
						wood.total = 0;
					...
					if (stone.total > 0) { gameLog('蠻族搶劫了石材'); }
					if (stone.total >= stolen){
						stone.total -= stolen;
					} else {
						stone.total = 0;
					...
					if (skins.total > 0) { gameLog('蠻族搶劫了獸皮'); }
					if (skins.total >= stolen){
						skins.total -= stolen;
					} else {
						skins.total = 0;
					...
					if (herbs.total > 0) { gameLog('蠻族搶劫了藥草'); }
					if (herbs.total >= stolen){
						herbs.total -= stolen;
					} else {
						herbs.total = 0;
					...
					if (ore.total > 0) { gameLog('蠻族搶劫了礦石'); }
					if (ore.total >= stolen){
						ore.total -= stolen;
					} else {
						ore.total = 0;
					...
					if (leather.total > 0) { gameLog('蠻族搶劫了皮革'); }
					if (leather.total >= stolen){
						leather.total -= stolen;
					} else {
						leather.total = 0;
					...
					if (metal.total > 0) { gameLog('蠻族搶劫了金屬'); }
					if (metal.total >= stolen){
						metal.total -= stolen;
					} else {
						metal.total = 0;
	...
	if (population.esiege > 0){
		...
		if (population.bandits > 0 || population.barbarians > 0){
			...
			if (upgrades.mathematics){ //Can we use them?
				gameLog('奪取' + prettify(population.esiege) + '個敵軍攻城塔.');
				...
				gameLog('摧毀了' + prettify(population.esiege) + '個敵軍攻城塔.');
			}
			population.esiege = 0;
		}
		updateMobs();
...
function toggleCustomIncrements(){
	...
	if(customIncrements){
		customIncrements = false;
		document.getElementById('toggleCustomIncrements').innerHTML = "允許自定義增加量";
		...
		document.getElementById('toggleCustomIncrements').innerHTML = "禁止自定義增加量";
